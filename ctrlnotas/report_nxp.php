...
class PDF extends FPDF {
    ...
    public function Header() {
        ...
        $this->Cell($w[2], 5, 'Auto', 1, 0, 'C');
        $this->Cell($w[2], 5, 'Hete', 1, 0, 'C');
        $this->Cell($w[2], 5, '20%', 1, 0, 'C');
        $this->Cell($w[2], 5, 'P.O', 1, 0, 'C');
        $this->Cell($w[2], 5, '30%', 1, 0, 'C');
    ...
    public function table($obj) {
        ...
        if (count($alumn) > 0 ){
            for ($i = 0; $i < count($alumn); $i++):
                ...
                $this->Cell($w[2], 4, sprintf("%.1F",$alumn[$i]['auto']), 1, 0, 'C');
                $this->Cell($w[2], 4, sprintf("%.1F",$alumn[$i]['hetero']), 1, 0, 'C');
                $this->Cell($w[2], 4, sprintf("%.1F",$alumn[$i]['eval']), 1, 0, 'C');
                $this->Cell($w[2], 4, sprintf("%.1F",$alumn[$i]['probj']), 1, 0, 'C');
                $this->Cell($w[2], 4, sprintf("%.1F",$alumn[$i]['probjp']), 1, 0, 'C');
