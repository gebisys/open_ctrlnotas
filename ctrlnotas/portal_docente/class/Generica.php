...
class Generica {
    ...
    public function getEnabledNotes($id) {
    	$sql	= "SELECT `enabled_period` AS `enabled` FROM `ctrl_periodos` WHERE `prdo_id` = '{$id}'";
    	$stmt	= $this->obj->Query($this->db, $sql);
    	return $this->obj->fetchAssoc($stmt);
    }
